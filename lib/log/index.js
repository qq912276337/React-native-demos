...
if (typeof DLLogFileMessageType == "undefined") {
    ...
    DLLogFileMessageType.log = '.log';
    DLLogFileMessageType.zip = '.zip';
}
...
export default class DLLogFileModule {
    ...
    start() {
            ...
            var fileName = date.toString() + DLLogFileMessageType.log;
    ...
    _diffTimerAction() {
        var nowTime = moment();
        this.currentDateFileName = (nowTime.format(kDayFormatter) + DLLogFileMessageType.log);
    ...
    _willCreateLogFileAtPath(path) {
        ...
        this._limitCountOfLogFiles((error, results) => {
            ...
            if (!error && results.length > 0 && this.autoUploadEnable == true) {
                for (var i = 0; i < results.length; i++) {
                    if (results[i].indexOf(DLLogFileMessageType.log) == -1) continue;
                    var zipfileName = results[i].split('.')[0] + DLLogFileMessageType.zip;
    ...
    _limitCountOfLogFiles(completeHandler) {
        this._allFiles((result) => {
            ...
            for (var i = 0; i < result.length; i++) {
                ...
                if (fileName.indexOf(DLLogFileMessageType.log) >= 0 || fileName.indexOf(DLLogFileMessageType.zip) >= 0) {
                    files.push(fileName);
                }
            }
